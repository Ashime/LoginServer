...
public class IniFile
{
    ...
    private static boolean mySql;             // MySQL enable value.
    private static boolean postgreSQL;        // PostgreSQL enable value.
    private static boolean msSQLServer;       // Microsoft SQL server enable value.
    ...
    // Parses all sections in the LoginServer.ini.
    private static void loginServerInfo()
    {
        try
        {
            // Opens the file for parsing.
            Wini ini = new Wini(new File("LoginServer.ini"));
            ...
            hmacKey = ini.get("SECURITY", "HMAC_KEY", String.class);
            ...
            // [SQL CONFIG] section.
            mySql = ini.get("SQL CONFIG", "MYSQL", boolean.class);
            postgreSQL = ini.get("SQL CONFIG", "POSTGRESQL", boolean.class);
            msSQLServer = ini.get("SQL CONFIG", "MS_SQLSERVER", boolean.class);
    ...
    private static void checkThreads()
    {
        if (clientAcceptingThreads <= 0)
        {
            ...
            Console.displayMessage("WARN", "Threads has been set to 1. See LoginServer.ini");
        }
